...
function PaginationButton(totalPages, maxPagesVisible = 10, currentPage = 1) {
  ...
  const paginationButtonContainer = document.createElement('nav');
  ...
  const createAndSetupButton = (label = '', cls = '', disabled = false, handleClick) => {
    ...
    if(label === 'prev') {
      buttonElement.innerHTML = '<i class="fas fa-arrow-left"></i>';
    } else if(label === 'next') {
      buttonElement.innerHTML = '<i class="fas fa-arrow-right"></i>';
    } else if(label === 'start') {
      buttonElement.innerHTML = '<i class="fas fa-step-backward"></i>';
    } else if(label === 'end') {
      buttonElement.innerHTML = '<i class="fas fa-step-forward"></i>';
    } else {
